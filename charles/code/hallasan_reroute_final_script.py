...
paths_df['출발_point'] = paths_df.apply(lambda r: Point(r['출발_lon'], r['출발_lat']), axis=1)
paths_df['도착_point'] = paths_df.apply(lambda r: Point(r['도착_lon'], r['도착_lat']), axis=1)
paths_df['direct_line'] = paths_df.apply(lambda r: LineString([r['출발_point'], r['도착_point']]), axis=1)
hallasan_paths = paths_df[paths_df['direct_line'].apply(lambda line: line.intersects(hallasan_union))].copy()
...
rerouted_df.to_csv("hallasan_rerouted_paths.csv", index=False)
...
for _, row in rerouted_df.iterrows():
    ...
    folium.CircleMarker([row['우회_lat'], row['우회_lon']], radius=3, color='orange', fill=True).add_to(m)

m.save("hallasan_rerouted_paths.html")
